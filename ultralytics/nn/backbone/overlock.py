...
__all__ = ["GDSAFusion", "overlock_b", "overlock_s", "overlock_t", "overlock_xt"]
...
class CTXDownsample(nn.Module):
    def __init__(self, dim, h_dim):
        ...
        self.x_proj = nn.Sequential(
            nn.Conv2d(dim, h_dim, kernel_size=3, stride=2, padding=1, bias=False), nn.BatchNorm2d(h_dim)
        )
...
class OverLoCK(nn.Module):
    """An Overview-first-Look-Closely-next ConvNet with Context-Mixing Dynamic Kernels https://arxiv.org/abs/2502.20087.
    """
    ...
    def forward(self, x):
        ...
        outputs.append(x)
        x, _ctx_cls = self.forward_sub_features(x, ctx)
        outputs.append(x)
