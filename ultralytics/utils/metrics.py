...
def get_inner_iou(box1, box2, xywh=True, eps=1e-7, ratio=0.7):
    def xyxy2xywh(x):
        """Convert bounding box coordinates from (x1, y1, x2, y2) format to (x, y, width, height) format where (x1, y1)
...
def bbox_inner_iou(
    ...
    if CIoU or DIoU or GIoU or EIoU or SIoU or ShapeIoU or PIoU or PIoU2:
        ...
        if (
            CIoU or DIoU or EIoU or SIoU or PIoU or PIoU2 or ShapeIoU
...
def wasserstein_loss(pred, target, eps=1e-7, constant=12.8):
    r"""`Implementation of paper `Enhancing Geometric Factors into Model Learning and Inference for Object Detection and
    Instance Segmentation <https://arxiv.org/abs/2005.03572>`_. Code is modified
    from https://github.com/Zzh-tju/CIoU.

    Args:
        pred (Tensor): Predicted bboxes of format (x_min, y_min, x_max, y_max), shape (n, 4).
...
class WiseIouLoss(torch.nn.Module):
    ...
    def forward(self, pred, target, ret_iou=False, ratio=1.0, d=0.0, u=0.95, **kwargs):
        ...
        if self.training:
            self.iou_mean.mul_(1 - self.momentum)
            # 确保 mean 不会低于 0.1，防止 beta 溢出
            current_mean = self["iou"].detach().mean()
            self.iou_mean.add_(self.momentum * current_mean)
            self.iou_mean.clamp_(min=0.1)
    ...
    def _scaled_loss(self, loss, iou=None):
        if isinstance(self.monotonous, bool):
            beta = (self["iou"].detach() if iou is None else iou) / (self.iou_mean + 1e-7)  # 防止除零

            if self.monotonous:
                loss *= beta.sqrt()
            else:
                # 限制 beta 的最大值，建议限制在 3-5 之间，防止 pow 函数爆炸
    ...
    def _PIoU(self):
        b1_x1, b1_y1, b1_x2, b1_y2 = self["pred"].chunk(4, -1)
        b2_x1, b2_y1, b2_x2, b2_y2 = self["target"].chunk(4, -1)
        _w1, _h1 = b1_x2 - b1_x1, b1_y2 - b1_y1 + 1e-7
        w2, h2 = b2_x2 - b2_x1, b2_y2 - b2_y1 + 1e-7
